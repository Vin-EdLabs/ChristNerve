import { pool } from '../db';
import { notifyChurchUsers } from '../routes/notifications';
...
const ranBirthday: Set<DayKey> = new Set();
const ranAbsent: Set<string> = new Set(); // churchId-weekKey
const ranBadges: Set<string> = new Set(); // YYYY-MM
...
async function runBirthdayAnniversaryAlerts() {
  const day = todayKey();
  if (ranBirthday.has(day)) return;
  // Only fire once per calendar day after 06:00 local (server local)
  if (new Date().getHours() < 6) return;

  try {
    const birthdays = await pool.query<MemberBrief>(
      `SELECT m.id, m.church_id, m.first_name, m.last_name, m.phone, m.whatsapp,
    ...
    const anniversaries = await pool.query<MemberBrief>(
      `SELECT m.id, m.church_id, m.first_name, m.last_name, m.phone, m.whatsapp,
              t.name AS church_name
       FROM church_members m
       JOIN church_tenants t ON t.id = m.church_id
    ...
    const byChurch = new Map<
      number,
      { churchName: string; bdays: MemberBrief[]; anniv: MemberBrief[] }
    >();

    for (const row of birthdays.rows) {
      const entry = byChurch.get(row.church_id) || {
        churchName: row.church_name || '',
        bdays: [] as MemberBrief[],
        anniv: [] as MemberBrief[],
      };
    ...
    for (const [churchId, data] of byChurch) {
      const names = [
        ...data.bdays.map(
          (m) => `${m.first_name} ${m.last_name} (birthday)`.trim()
        ),
        ...data.anniv.map(
          (m) => `${m.first_name} ${m.last_name} (anniversary)`.trim()
        ),
      ];
    ...
    ranBirthday.add(day);
    console.log(
      `[reminder] birthday/anniversary alerts: ${birthdays.rows.length} bdays, ${anniversaries.rows.length} anniv`
    );
  } catch (err) {
...
async function runWeeklyAbsentDetection() {
  ...
  try {
    const churches = await pool.query(
      `SELECT id, name FROM church_tenants WHERE is_active = true OR is_active IS NULL`
    );

    for (const church of churches.rows) {
      const key = `${church.id}-${wk}`;
      if (ranAbsent.has(key)) continue;
      ...
      if (absent.rows.length > 0) {
        ...
        await notifyChurchUsers({
          ...
          body: `${absent.rows.length} member(s) missed the last 2 services: ${preview}${
            absent.rows.length > 6 ? '…' : ''
          }`,
          link: '/follow-up',
        });
      }
      ...
      ranAbsent.add(key);
      console.log(
        `[reminder] absent check church ${church.id}: ${absent.rows.length}`
      );
    }
  } catch (err) {
...
async function runMonthlyMilestoneBadges() {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO church_member_badges (church_id, member_id, badge_key, badge_label, awarded_at)
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log(
        ...
        result.rows.map(
          (r) => `${r.church_name}: ${r.title} @ ${r.start_datetime}`
        )
      );
    }
  } catch (err) {
