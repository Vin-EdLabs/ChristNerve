...
function actor(req: Request) {
  ...
  return { type: accountType as 'member' | 'staff', id: Number(req.churchUser!.id) };
}
  ...
  try {
    ...
    if (listingId && !Number.isNaN(listingId)) {
      const listing = await pool.query(
        `SELECT id, member_id, title, is_active
    ...
    const seller = await pool.query(
      `SELECT id, first_name, last_name FROM church_members
       WHERE id = $1 AND church_id = $2 AND membership_status = 'active'`,
      [sellerId, churchId]
    );
    ...
    let conv = await pool.query(
      `SELECT * FROM market_conversations
       WHERE church_id = $1
         AND buyer_type = $2
         AND buyer_id = $3
         AND seller_member_id = $4
         AND COALESCE(listing_id, 0) = COALESCE($5::int, 0)`,
      [churchId, me.type, me.id, sellerId, listingId]
    );
    ...
    if (listingId && Number(conversation.listing_id) !== listingId) {
      await pool.query(
        `UPDATE market_conversations SET listing_id = $1 WHERE id = $2`,
    ...
    const existingMsg = await pool.query(
      `SELECT id FROM market_messages WHERE conversation_id = $1 LIMIT 1`,
      [conversation.id]
    );

    if (existingMsg.rows.length === 0) {
      await pool.query(
        `INSERT INTO market_messages (conversation_id, sender_type, sender_id, body)
         VALUES ($1, 'buyer', $2, $3)`,
        [conversation.id, me.id, firstBody]
      );
      await pool.query(
        `UPDATE market_conversations SET last_message_at = NOW() WHERE id = $1`,
        [conversation.id]
      );
...
router.get('/conversations/:id/messages', async (req: Request, res: Response) => {
  try {
    ...
    const c = conv.rows[0];
    const isBuyer =
      c.buyer_type === me.type && Number(c.buyer_id) === me.id;
    const isSeller =
      me.type === 'member' && Number(c.seller_member_id) === me.id;
    if (!isBuyer && !isSeller) {
    ...
    await pool.query(
      `UPDATE market_messages SET is_read = true
       WHERE conversation_id = $1
         AND is_read = false
         AND NOT (sender_type = $2 AND sender_id = $3)`,
      [id, isBuyer ? 'buyer' : 'seller', me.id]
    );
