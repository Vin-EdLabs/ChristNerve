...
async function nextReceiptNumber(): Promise<string> {
  ...
  const result = await pool.query(
    `SELECT receipt_number FROM church_giving
...
router.get('/giving/summary', async (req: Request, res: Response) => {
  try {
    ...
    const byType = await pool.query(
      `SELECT giving_type, COALESCE(SUM(amount), 0)::numeric AS total, COUNT(*)::int AS count
       FROM church_giving
       WHERE church_id = $1
    ...
    const thisMonthTotal = await pool.query(
...
router.get('/giving', async (req: Request, res: Response) => {
  try {
    ...
    const conditions: string[] = ['g.church_id = $1'];
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*)::int AS total FROM church_giving g WHERE ${where}`,
      params
    );
...
router.get('/expenses', async (req: Request, res: Response) => {
  try {
    ...
    const conditions: string[] = ['e.church_id = $1'];
    const params: unknown[] = [churchId];
    let idx = 2;
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*)::int AS total FROM church_expenses e WHERE ${where}`,
      params
    );
...
router.get('/report', async (req: Request, res: Response) => {
  try {
    ...
    const incomeByType = await pool.query(
      `SELECT giving_type, COALESCE(SUM(amount), 0)::numeric AS total
       FROM church_giving
       WHERE church_id = $1
    ...
    const incomeTotal = incomeByType.rows.reduce(
      (sum, r) => sum + parseFloat(r.total),
      0
    );
    const expenseTotal = expenseByCategory.rows.reduce(
      (sum, r) => sum + parseFloat(r.total),
      0
    );
