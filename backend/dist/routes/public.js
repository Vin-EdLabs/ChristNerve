 ...
 */
router.post('/register-church', upload_1.upload.single('logo'), async (req, res) => {
    try {
        const body = req.body || {};
        ...
        let churchSlug = normalizeSlug(body.slug || (0, slug_1.generateSlug)(name));
        ...
        // Ensure unique slug
        const existing = await db_1.pool.query('SELECT id FROM church_tenants WHERE slug = $1', [churchSlug]);
        if (existing.rows.length > 0) {
            churchSlug = `${churchSlug}-${Date.now().toString(36).slice(-4)}`;
        }
        const logoUrl = (0, upload_1.uploadedFilePublicUrl)(req.file) || null;
        const result = await db_1.pool.query(`INSERT INTO church_tenants (
       ...
       RETURNING id, name, slug, logo_url, city, region, phone, email, tagline,
        ...
        res.status(201).json({
            ok: true,
            message: 'Registration received. ChristNerve will review your request shortly.',
            church: {
                id: church.id,
                name: church.name,
                slug: church.slug,
...
router.get('/manifest', async (req, res) => {
    try {
        ...
        const shortName = church.short_name ||
 ...
 */
router.get('/church/:slug/events', async (req, res) => {
    try {
        const slug = req.params.slug.toLowerCase();
        const church = await db_1.pool.query('SELECT id FROM church_tenants WHERE slug = $1 AND is_active = true', [slug]);
        if (church.rows.length === 0) {
            res.status(404).json({ error: 'Church not found' });
            return;
        }
        const events = await db_1.pool.query(`SELECT id, title, description, event_type, start_datetime, end_datetime,
              location, banner_url
       FROM church_events
       WHERE church_id = $1 AND is_public = true AND start_datetime >= NOW() - INTERVAL '1 day'
       ORDER BY start_datetime ASC`, [church.rows[0].id]);
 ...
 */
router.get('/church/:slug/market', async (req, res) => {
    try {
        const slug = req.params.slug.toLowerCase();
        const church = await db_1.pool.query(`SELECT id, name, slug, logo_url, tagline, city
       FROM church_tenants WHERE slug = $1 AND is_active = true`, [slug]);
        if (church.rows.length === 0) {
            res.status(404).json({ error: 'Church not found' });
            return;
        }
        ...
        const [categoryCounts, featured, totals] = await Promise.all([
            db_1.pool.query(`SELECT c.id, c.name, c.slug, c.icon, COUNT(l.id)::int AS listing_count
         ...
         HAVING COUNT(l.id) > 0
         ORDER BY c.display_order ASC`, [churchId]),
            db_1.pool.query(`SELECT l.id, l.title, l.slug, l.price_min, l.price_max, l.price_label,
