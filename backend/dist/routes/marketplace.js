...
async function uniqueListingSlug(base, memberId) {
    ...
    while (true) {
        ...
        const result = await db_1.pool.query('SELECT id FROM market_listings WHERE slug = $1', [slug]);
...
router.get('/categories', async (_req, res) => {
    try {
        const result = await db_1.pool.query('SELECT * FROM market_categories ORDER BY display_order ASC, name ASC');
...
router.get('/listings', async (req, res) => {
    try {
        const churchSlug = req.query.church_slug ||
        ...
        const tenant = await db_1.pool.query('SELECT id FROM church_tenants WHERE slug = $1 AND is_active = true', [churchSlug.toLowerCase()]);
        ...
        const countResult = await db_1.pool.query(`SELECT COUNT(*)::int AS total
       FROM market_listings l
       LEFT JOIN market_categories c ON c.id = l.category_id
       WHERE ${where}`, params);
        const dataResult = await db_1.pool.query(`SELECT l.*,
...
router.get('/listings/:slug', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`UPDATE market_listings
       ...
       RETURNING *`, [slug]);
        ...
        const [images, category, member, church, reviews, more] = await Promise.all([
            db_1.pool.query(`SELECT * FROM market_listing_images
         WHERE listing_id = $1
         ORDER BY is_primary DESC, display_order ASC`, [listing.id]),
            db_1.pool.query('SELECT * FROM market_categories WHERE id = $1', [
                listing.category_id,
            ]),
            db_1.pool.query(`SELECT id, first_name, last_name, avatar_url, is_verified,
                marketplace_slug, phone, whatsapp, occupation, city, membership_date
         FROM church_members WHERE id = $1`, [listing.member_id]),
            db_1.pool.query(`SELECT id, name, slug, logo_url, tagline, city, denomination
         FROM church_tenants WHERE id = $1`, [listing.church_id]),
            db_1.pool.query(`SELECT r.*, m.first_name, m.last_name
 ...
 */
router.get('/storefront/:memberSlug', async (req, res) => {
    try {
        const { memberSlug } = req.params;
        const memberResult = await db_1.pool.query(`SELECT m.*,
       ...
       WHERE m.marketplace_slug = $1`, [memberSlug]);
        if (memberResult.rows.length === 0) {
            res.status(404).json({ error: 'Storefront not found' });
            return;
        }
        const member = memberResult.rows[0];
        const listings = await db_1.pool.query(`SELECT l.*,
              c.name AS category_name, c.slug AS category_slug,
              (
                SELECT image_url FROM market_listing_images i
                WHERE i.listing_id = l.id
       ...
       ORDER BY l.is_featured DESC, l.created_at DESC`, [member.id]);
        res.json({
            member: {
                id: member.id,
 ...
 */
router.get('/my-listings', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT l.*,
         ...
         ORDER BY l.created_at DESC`, [churchId, memberId]);
 ...
 */
router.get('/my-listings/:id', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        ...
        const result = await db_1.pool.query(`SELECT l.*,
         ...
         FROM market_listings l
         LEFT JOIN market_categories c ON c.id = l.category_id
         WHERE l.id = $1 AND l.church_id = $2`, [id, churchId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Listing not found' });
            return;
        }
        ...
        const images = await db_1.pool.query(`SELECT * FROM market_listing_images
         WHERE listing_id = $1
         ORDER BY is_primary DESC, display_order ASC`, [id]);
        res.json({
            ...listing,
            images: images.rows,
            primary_image: listing.primary_image || images.rows[0]?.image_url || null,
        });
    }
    catch (err) {
 ...
 */
router.post('/listings', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        const { member_id, category_id, title, description, price_min, price_max, price_label, location, whatsapp, phone, is_featured, } = req.body;
        ...
        if (req.accountType === 'member') {
            ...
            const member = await db_1.pool.query('SELECT id FROM church_members WHERE id = $1 AND church_id = $2', [resolvedMemberId, churchId]);
            if (member.rows.length === 0) {
                res.status(404).json({ error: 'Member not found in this church' });
                return;
            }
        }
        const listingMemberId = resolvedMemberId;
        const baseSlug = (0, slug_1.generateSlug)(title) || 'listing';
        const slug = await uniqueListingSlug(baseSlug, listingMemberId);
        const result = await db_1.pool.query(`INSERT INTO market_listings (
 ...
 */
router.put('/listings/:id', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
        ...
        const existing = await db_1.pool.query('SELECT * FROM market_listings WHERE id = $1 AND church_id = $2', [id, churchId]);
        ...
        const b = req.body;
        const result = await db_1.pool.query(`UPDATE market_listings SET
           category_id = $1,
           title = $2,
           description = $3,
           price_min = $4,
 ...
 */
router.delete('/listings/:id', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
        ...
        const existing = await db_1.pool.query('SELECT id, member_id FROM market_listings WHERE id = $1 AND church_id = $2', [id, churchId]);
        if (existing.rows.length === 0) {
            res.status(404).json({ error: 'Listing not found' });
            return;
        }
 ...
 */
router.post('/listings/:id/images', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, upload_1.upload.array('images', 5), async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        const id = parseInt(req.params.id, 10);
        ...
        const listing = await db_1.pool.query('SELECT id, member_id FROM market_listings WHERE id = $1 AND church_id = $2', [id, churchId]);
        ...
        const existingCount = await db_1.pool.query('SELECT COUNT(*)::int AS count FROM market_listing_images WHERE listing_id = $1', [id]);
        const startOrder = existingCount.rows[0].count;
        const hasPrimary = await db_1.pool.query('SELECT id FROM market_listing_images WHERE listing_id = $1 AND is_primary = true', [id]);
        const inserted = [];
        for (let i = 0; i < files.length; i++) {
            const file = files[i];
            const imageUrl = `/uploads/church/${file.filename}`;
            const isPrimary = hasPrimary.rows.length === 0 && i === 0;
            const row = await db_1.pool.query(`INSERT INTO market_listing_images (listing_id, image_url, is_primary, display_order)
           VALUES ($1, $2, $3, $4)
           RETURNING *`, [id, imageUrl, isPrimary, startOrder + i]);
 ...
 */
router.post('/reviews', churchTenant_1.requireChurchTenant, churchAuth_1.requireChurchAuth, async (req, res) => {
    try {
        const churchId = req.churchTenant.id;
        ...
        const listing = await db_1.pool.query('SELECT id FROM market_listings WHERE id = $1 AND church_id = $2', [listing_id, churchId]);
        if (listing.rows.length === 0) {
            res.status(404).json({ error: 'Listing not found' });
            return;
        }
        ...
        if (!reviewerId || Number.isNaN(reviewerId)) {
            const first = await db_1.pool.query(`SELECT id FROM church_members
           WHERE church_id = $1 AND membership_status = 'active'
           ORDER BY id ASC LIMIT 1`, [churchId]);
            reviewerId = first.rows[0]?.id || null;
        }
        const result = await db_1.pool.query(`INSERT INTO market_reviews (listing_id, reviewer_member_id, rating, comment)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (listing_id, reviewer_member_id)
         DO UPDATE SET rating = EXCLUDED.rating, comment = EXCLUDED.comment
         RETURNING *`, [listing_id, reviewerId, ratingNum, comment || null]);
