...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('SELECT * FROM church_events WHERE id = $1 AND church_id = $2', [id, churchId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const { title, description, event_type, start_datetime, end_datetime, location, banner_url, is_public, } = req.body;
        if (!title || !start_datetime) {
            res.status(400).json({ error: 'title and start_datetime are required' });
            return;
        }
        const result = await db_1.pool.query(`INSERT INTO church_events (
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT * FROM church_events WHERE id = $1 AND church_id = $2', [id, churchId]);
        ...
        const b = req.body;
        const result = await db_1.pool.query(`UPDATE church_events SET
         title = $1,
         description = $2,
         event_type = $3,
         start_datetime = $4,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT id, title FROM church_events WHERE id = $1 AND church_id = $2', [id, churchId]);
        if (existing.rows.length === 0) {
            res.status(404).json({ error: 'Event not found' });
            return;
        }
        await db_1.pool.query('DELETE FROM church_events WHERE id = $1 AND church_id = $2', [id, churchId]);
        await (0, audit_1.writeAudit)({
            churchId,
            actorType: 'staff',
