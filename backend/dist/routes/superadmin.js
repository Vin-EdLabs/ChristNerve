...
router.use(churchAuth_1.requireSuperAdmin);
/**
 ...
 */
router.post('/churches', upload_1.upload.single('logo'), async (req, res) => {
    const client = await db_1.pool.connect();
    try {
        const body = req.body || {};
        const { name, slug, tagline, description, banner_url, address, city, region, phone, email, denomination, founded_year, subscription_plan, subscription_status, subscription_amount, next_billing_date, admin_first_name, admin_last_name, admin_email, admin_password, admin_phone, admin_role, } = body;
        const logo_url = (0, upload_1.uploadedFilePublicUrl)(req.file) || body.logo_url || null;
        if (!name) {
            res.status(400).json({ error: 'Church name is required' });
            return;
        }
        const churchSlug = String(slug || (0, slug_1.generateSlug)(name))
            .toLowerCase()
            .replace(/[^a-z0-9-]/g, '')
            .replace(/-+/g, '-')
            .replace(/^-|-$/g, '');
        ...
        const result = await client.query(`INSERT INTO church_tenants (
       ...
       RETURNING *`, [
        ...
        const passwordHash = await bcryptjs_1.default.hash(String(admin_password), 10);
        const adminResult = await client.query(`INSERT INTO church_users (
 ...
 */
router.post('/churches/:id/logo', upload_1.upload.single('logo'), async (req, res) => {
    try {
 ...
 */
router.post('/churches/:id/approve', async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
            res.status(400).json({ error: 'Invalid church id' });
        ...
        const result = await db_1.pool.query(`UPDATE church_tenants
       SET is_active = false,
 ...
 */
router.post('/churches/:id/setup', async (req, res) => {
    const client = await db_1.pool.connect();
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
        ...
        const body = req.body || {};
        const existing = await client.query('SELECT * FROM church_tenants WHERE id = $1', [id]);
        ...
        const address = body.address !== undefined ? String(body.address || '').trim() || null : church.address;
        const denomination = body.denomination !== undefined
        ...
        const secondaryColor = String(body.secondary_color || church.secondary_color || '#C4A035').trim();
        const shortName = body.short_name !== undefined
            ? String(body.short_name || '').trim() || null
            : church.short_name;
        const adminName = String(body.admin_name || '').trim();
        const adminFirst = String(body.admin_first_name || '').trim() ||
            (adminName ? adminName.split(/\s+/)[0] : '') ||
            'Primary';
        const adminLast = String(body.admin_last_name || '').trim() ||
        ...
        await client.query('BEGIN');
        const updated = await client.query(`UPDATE church_tenants
       SET name = $1,
           slug = $2,
           tagline = $3,
           city = $4,
       ...
       ON CONFLICT (church_id, email) DO UPDATE SET
         ...
         role = 'super-admin',
         is_active = true
       RETURNING id, church_id, first_name, last_name, email, username, phone, role, created_at`, [id, adminFirst, adminLast, adminEmail, adminUsername, passwordHash, adminPhone]);
        await client.query('COMMIT');
        res.json({
            ok: true,
 ...
 */
router.post('/churches/:id/reject', async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
            res.status(400).json({ error: 'Invalid church id' });
...
router.put('/churches/:id', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT * FROM church_tenants WHERE id = $1', [id]);
        if (existing.rows.length === 0) {
            res.status(404).json({ error: 'Church not found' });
            return;
        }
        ...
        const b = req.body;
        const result = await db_1.pool.query(`UPDATE church_tenants SET
         name = $1,
         slug = $2,
         tagline = $3,
         description = $4,
       ...
       RETURNING *`, [
        ...
        const adminUsername = b.admin_username !== undefined
            ? String(b.admin_username || '').trim().toLowerCase()
            : '';
        const adminPassword = b.admin_password !== undefined ? String(b.admin_password || '') : '';
        const adminEmail = b.admin_email !== undefined
        ...
        if (adminName || adminUsername || adminPassword || b.admin_email !== undefined) {
            const primary = await db_1.pool.query(`SELECT * FROM church_users
         WHERE church_id = $1
         ...
         ORDER BY
           ...
           id ASC
         LIMIT 1`, [id]);
            const first = adminName.split(/\s+/)[0] ||
                primary.rows[0]?.first_name ||
                'Primary';
            const last = adminName.split(/\s+/).slice(1).join(' ') ||
            ...
            if (primary.rows.length > 0) {
                ...
                await db_1.pool.query(`UPDATE church_users SET
             first_name = $1,
             last_name = $2,
             email = $3,
           ...
           WHERE id = $6 AND church_id = $7`, [
                ...
                const passwordHash = await bcryptjs_1.default.hash(adminPassword, 10);
                await db_1.pool.query(`INSERT INTO church_users (
 ...
 */
router.get('/churches/:id/stats', async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
            res.status(400).json({ error: 'Invalid church id' });
        ...
        const church = await db_1.pool.query('SELECT * FROM church_tenants WHERE id = $1', [id]);
        if (church.rows.length === 0) {
            res.status(404).json({ error: 'Church not found' });
            return;
        }
        const stats = await db_1.pool.query(`SELECT
         ...
         (SELECT COALESCE(AVG(total_count), 0)::numeric(10,1) FROM church_attendance
           WHERE church_id = $1
             AND service_date >= (CURRENT_DATE - INTERVAL '3 months')) AS avg_attendance`, [id]);
...
router.post('/notifications', async (req, res) => {
    try {
        ...
        const churchId = church_id !== undefined && church_id !== null && church_id !== ''
        ...
        if (churchId !== null) {
            const church = await db_1.pool.query('SELECT id FROM church_tenants WHERE id = $1', [churchId]);
            if (church.rows.length === 0) {
                res.status(404).json({ error: 'Church not found' });
                return;
            }
            const targetType = user_type === 'member' || user_type === 'staff' ? user_type : null;
            const targetUserId = user_id !== undefined && user_id !== null && user_id !== ''
            ...
            const id = targetUserId == null && !targetType
            ...
            const row = await db_1.pool.query('SELECT * FROM notifications WHERE id = $1', [
                id,
            ]);
...
router.get('/audit', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT a.*, t.name AS church_name
       FROM audit_logs a
...
router.get('/notifications/health', async (_req, res) => {
    try {
        const [churchTokens, platformTokens, notifs] = await Promise.all([
            db_1.pool.query(`SELECT COUNT(*)::int AS c FROM device_tokens WHERE church_id IS NOT NULL`),
            db_1.pool.query(`SELECT COUNT(*)::int AS c FROM device_tokens WHERE user_type = 'superadmin'`),
            db_1.pool.query(`SELECT COUNT(*)::int AS c FROM notifications
         WHERE created_at >= NOW() - INTERVAL '24 hours'`),
        ]);
        res.json({
            fcm_configured: (0, fcm_1.isFcmConfigured)(),
