...
router.get('/', async (req, res) => {
    try {
       ...
       WHERE a.church_id = $1
       ORDER BY a.is_pinned DESC, a.publish_date DESC, a.id DESC`, [churchId]);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('SELECT * FROM church_announcements WHERE id = $1 AND church_id = $2', [id, churchId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const { title, body, audience, department_id, is_pinned, publish_date, } = req.body;
        if (!title || !body) {
            res.status(400).json({ error: 'title and body are required' });
            return;
        }
        const result = await db_1.pool.query(`INSERT INTO church_announcements (
       ...
       RETURNING *`, [
        ...
        const preview = String(body).length > 140 ? `${String(body).slice(0, 137)}…` : String(body);
        try {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT * FROM church_announcements WHERE id = $1 AND church_id = $2', [id, churchId]);
        ...
        const b = req.body;
        const result = await db_1.pool.query(`UPDATE church_announcements SET
         title = $1,
         body = $2,
         audience = $3,
         department_id = $4,
...
router.post('/:id/pin', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT is_pinned FROM church_announcements WHERE id = $1 AND church_id = $2', [id, churchId]);
        if (existing.rows.length === 0) {
            res.status(404).json({ error: 'Announcement not found' });
            return;
        }
        const nextPinned = req.body.is_pinned !== undefined
            ? Boolean(req.body.is_pinned)
            : !existing.rows[0].is_pinned;
        const result = await db_1.pool.query(`UPDATE church_announcements
       SET is_pinned = $1
       WHERE id = $2 AND church_id = $3
       RETURNING *`, [nextPinned, id, churchId]);
        res.json(result.rows[0]);
    }
    catch (err) {
