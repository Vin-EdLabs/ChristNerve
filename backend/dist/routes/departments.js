...
async function isDeptLeader(churchId, departmentId, memberId) {
    const r = await db_1.pool.query(`SELECT 1 FROM church_departments
     ...
     WHERE department_id = $1 AND church_id = $2 AND member_id = $3
       AND role = 'leader'
     LIMIT 1`, [departmentId, churchId, memberId]);
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT d.*,
...
router.get('/mine', async (req, res) => {
    try {
        ...
        const member = await db_1.pool.query(`SELECT department, ministry, cell_group, membership_date
       FROM church_members WHERE id = $1 AND church_id = $2`, [memberId, churchId]);
        if (member.rows.length === 0) {
            res.status(404).json({ error: 'Member not found' });
            return;
        }
        const profile = member.rows[0];
        let depts = await db_1.pool.query(`SELECT d.*,
              m.first_name AS leader_first_name,
              m.last_name AS leader_last_name,
              dm.role AS my_role,
              (
       ...
       ORDER BY d.name ASC`, [memberId, churchId]);
        ...
        if (depts.rows.length === 0 && profile.department) {
         ...
         LIMIT 1`, [churchId, String(profile.department).split(',')[0].trim()]);
            depts = { ...fallback, rows: fallback.rows };
        }
        const departments = [];
        for (const d of depts.rows) {
            const [roster, posts] = await Promise.all([
                db_1.pool.query(`SELECT cm.id, cm.first_name, cm.last_name, cm.avatar_url, cm.phone,
           ...
           ORDER BY
             ...
             cm.last_name, cm.first_name`, [d.id]),
                db_1.pool.query(`SELECT p.*,
           ...
           LIMIT 30`, [d.id]),
            ...
            const canPost = d.leader_member_id === memberId ||
                d.my_role === 'leader' ||
...
router.post('/:id/posts', async (req, res) => {
    try {
        ...
        const type = ['update', 'meeting', 'event'].includes(post_type)
            ? post_type
            : 'update';
        const result = await db_1.pool.query(`INSERT INTO church_department_posts (
...
router.delete('/:id/posts/:postId', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`DELETE FROM church_department_posts
       WHERE id = $1 AND department_id = $2 AND church_id = $3
       RETURNING id`, [postId, departmentId, churchId]);
...
router.get('/:id', async (req, res) => {
    try {
       ...
       FROM church_departments d
       LEFT JOIN church_members m ON m.id = d.leader_member_id
       WHERE d.id = $1 AND d.church_id = $2`, [id, churchId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Department not found' });
            return;
        }
        const members = await db_1.pool.query(`SELECT cm.id, cm.first_name, cm.last_name, cm.member_number, cm.phone,
              cm.membership_status, cm.avatar_url, dm.role
       FROM church_department_members dm
       JOIN church_members cm ON cm.id = dm.member_id
       WHERE dm.department_id = $1 AND dm.church_id = $2
       ORDER BY cm.last_name, cm.first_name`, [id, churchId]);
        const posts = await db_1.pool.query(`SELECT p.*,
              a.first_name AS author_first_name,
              a.last_name AS author_last_name
       FROM church_department_posts p
       LEFT JOIN church_members a ON a.id = p.author_member_id
...
router.post('/', async (req, res) => {
    try {
        ...
        if (leader_member_id) {
            const member = await db_1.pool.query('SELECT id FROM church_members WHERE id = $1 AND church_id = $2', [leader_member_id, churchId]);
            if (member.rows.length === 0) {
                res.status(404).json({ error: 'Leader member not found' });
                return;
            }
        }
        const result = await db_1.pool.query(`INSERT INTO church_departments (church_id, name, description, leader_member_id)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [churchId, name, description || null, leader_member_id || null]);
        const dept = result.rows[0];
        if (leader_member_id) {
            await db_1.pool.query(`INSERT INTO church_department_members (church_id, department_id, member_id, role)
         VALUES ($1, $2, $3, 'leader')
         ON CONFLICT (department_id, member_id) DO UPDATE SET role = 'leader'`, [churchId, dept.id, leader_member_id]);
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await db_1.pool.query('SELECT * FROM church_departments WHERE id = $1 AND church_id = $2', [id, churchId]);
        ...
        if (b.leader_member_id) {
            const member = await db_1.pool.query('SELECT id FROM church_members WHERE id = $1 AND church_id = $2', [b.leader_member_id, churchId]);
            if (member.rows.length === 0) {
                res.status(404).json({ error: 'Leader member not found' });
                return;
            }
        }
        const result = await db_1.pool.query(`UPDATE church_departments SET
         name = $1,
         description = $2,
         leader_member_id = $3
       ...
       RETURNING *`, [
        ...
        const leaderId = b.leader_member_id !== undefined ? b.leader_member_id : cur.leader_member_id;
        if (leaderId) {
            await db_1.pool.query(`INSERT INTO church_department_members (church_id, department_id, member_id, role)
         VALUES ($1, $2, $3, 'leader')
         ON CONFLICT (department_id, member_id) DO UPDATE SET role = 'leader'`, [churchId, id, leaderId]);
        }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('DELETE FROM church_departments WHERE id = $1 AND church_id = $2 RETURNING id', [id, churchId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Department not found' });
            return;
        }
